...
//----------------------------------------------------------------------------------------
INT_PTR VDP::VRAMView::WndProcDialog(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	switch(msg)
	{
	case WM_INITDIALOG:
...
//Event handlers
//----------------------------------------------------------------------------------------
INT_PTR VDP::VRAMView::msgWM_INITDIALOG(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
...
//----------------------------------------------------------------------------------------
INT_PTR VDP::VRAMView::msgWM_CLOSE(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	DestroyWindow(hwnd);
...
//----------------------------------------------------------------------------------------
INT_PTR VDP::VRAMView::msgWM_COMMAND(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
...
//----------------------------------------------------------------------------------------
LRESULT CALLBACK VDP::VRAMView::WndProcRenderStatic(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
...
//----------------------------------------------------------------------------------------
LRESULT VDP::VRAMView::WndProcRender(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	switch(msg)
	{
	case WM_CREATE:
...
//----------------------------------------------------------------------------------------
LRESULT VDP::VRAMView::msgRenderWM_CREATE(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
...
//----------------------------------------------------------------------------------------
LRESULT VDP::VRAMView::msgRenderWM_CLOSE(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
...
//----------------------------------------------------------------------------------------
LRESULT VDP::VRAMView::msgRenderWM_TIMER(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	//Constants
	...
	if((blockSize == BLOCKSIZE_8X16) || ((blockSize == BLOCKSIZE_AUTO) && (device->GetInterlaceMode() == INTERLACE_DOUBLE)))
	{
		blockPixelSizeY = 16;
	...
	for(unsigned int line = 0; line < height; ++line)
	{
		for(unsigned int xpos = 0; xpos < width; ++xpos)
		{
			...
			Data blockData(8);
			device->vram->ReadCommitted(blockStart + (blockOffset / 2), blockData);
			...
			else
			{
				...
				color = device->GetPaletteColor(paletteIndex);
				r = color.GetR8Bit();
				g = color.GetG8Bit();
				b = color.GetB8Bit();
			}
...
//----------------------------------------------------------------------------------------
LRESULT VDP::VRAMView::msgRenderWM_MOUSEMOVE(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	if(!detailsVisible)
	{
	...
	unsigned int blockPixelSizeY = 8;
	unsigned int blockSizeForDisplay = 0x20;
	detailsBlock16 = (blockSize == BLOCKSIZE_8X16) || ((blockSize == BLOCKSIZE_AUTO) && (device->GetInterlaceMode() == INTERLACE_DOUBLE));
	if(detailsBlock16)
	{
		blockPixelSizeY = 16;
		blockSizeForDisplay = 0x40;
...
//----------------------------------------------------------------------------------------
LRESULT VDP::VRAMView::msgRenderWM_MOUSELEAVE(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	detailsVisible = false;
...
//----------------------------------------------------------------------------------------
INT_PTR CALLBACK VDP::VRAMView::WndProcDetailsStatic(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	//Obtain the object pointer
	VRAMView* state = (VRAMView*)GetWindowLongPtr(hwnd, GWLP_USERDATA);
...
//----------------------------------------------------------------------------------------
INT_PTR VDP::VRAMView::WndProcDetails(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	switch(msg)
	{
	case WM_INITDIALOG:
...
//----------------------------------------------------------------------------------------
INT_PTR VDP::VRAMView::msgDetailsWM_INITDIALOG(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	SetTimer(hwnd, 1, 200, NULL);
...
//----------------------------------------------------------------------------------------
INT_PTR VDP::VRAMView::msgDetailsWM_CLOSE(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	KillTimer(hwnd, 1);
	DestroyWindow(hwnd);
...
//----------------------------------------------------------------------------------------
INT_PTR VDP::VRAMView::msgDetailsWM_TIMER(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
