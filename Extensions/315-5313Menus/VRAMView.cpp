...
:ViewBase(auiManager, apresenter), presenter(apresenter), model(amodel)
{
	...
	SetDialogTemplateSettings(apresenter.GetUnqualifiedViewTitle(), GetAssemblyHandle(), MAKEINTRESOURCE(IDD_VDP_VRAM));
	SetDialogViewType();
}
...
//----------------------------------------------------------------------------------------
INT_PTR VRAMView::WndProcDialog(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	switch(msg)
	{
	case WM_INITDIALOG:
...
//Event handlers
//----------------------------------------------------------------------------------------
INT_PTR VRAMView::msgWM_INITDIALOG(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	...
	//Create the window
	hwndRender = CreateWindowEx(0, L"VRAM Render Child", L"VRAM Render Child", WS_CHILD, markerPos.x, markerPos.y, width, height, hwnd, NULL, GetAssemblyHandle(), this);
...
//----------------------------------------------------------------------------------------
INT_PTR VRAMView::msgWM_COMMAND(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
...
LRESULT VRAMView::msgRenderWM_TIMER(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	...
	if(obtainedVRAMData)
	{
		...
		if((blockSize == BLOCKSIZE_8X16) || ((blockSize == BLOCKSIZE_AUTO) && (model.RegGetLSM0()) && (model.RegGetLSM1())))
		{
			blockPixelSizeY = 16;
