...
//Constructors
//----------------------------------------------------------------------------------------
SpriteListDetailsView::SpriteListDetailsView(IUIManager& auiManager, SpriteListDetailsViewPresenter& apresenter, IS315_5313& amodel, unsigned int aspriteIndex)
:ViewBase(auiManager, apresenter), presenter(apresenter), model(amodel), initializedDialog(false), currentControlFocus(0)
{
	...
	SetDialogTemplateSettings(apresenter.GetUnqualifiedViewTitle(), GetAssemblyHandle(), MAKEINTRESOURCE(IDD_S315_5313_SPRITELIST_DETAILS));
	SetDialogViewType();
}
...
//----------------------------------------------------------------------------------------
void SpriteListDetailsView::SetSpriteIndex(unsigned int aspriteIndex)
{
	spriteIndex = aspriteIndex;
}
...
//----------------------------------------------------------------------------------------
INT_PTR SpriteListDetailsView::WndProcDialog(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
...
//Event handlers
//----------------------------------------------------------------------------------------
INT_PTR SpriteListDetailsView::msgWM_INITDIALOG(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	spriteMapping = model.GetSpriteMappingTableEntry(spriteIndex);
...
//----------------------------------------------------------------------------------------
INT_PTR SpriteListDetailsView::msgWM_DESTROY(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	KillTimer(hwnd, 1);
...
//----------------------------------------------------------------------------------------
INT_PTR SpriteListDetailsView::msgWM_TIMER(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	spriteMapping = model.GetSpriteMappingTableEntry(spriteIndex);
...
//----------------------------------------------------------------------------------------
INT_PTR SpriteListDetailsView::msgWM_COMMAND(HWND hwnd, WPARAM wparam, LPARAM lparam)
{
	if(HIWORD(wparam) == BN_CLICKED)
	{
		...
		model.SetSpriteMappingTableEntry(spriteIndex, spriteMapping, true);
	}
	...
	else if((HIWORD(wparam) == EN_KILLFOCUS) && initializedDialog)
	{
		...
		model.SetSpriteMappingTableEntry(spriteIndex, spriteMapping, writeSeparatedData);
	}
