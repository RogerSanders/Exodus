...
//----------------------------------------------------------------------------------------
template<class T> OpcodeTable<T>::~OpcodeTable()
{
	for(std::set<T*>::iterator i = opcodeSet.begin(); i != opcodeSet.end(); ++i)
	{
		delete *i;
	}
}
...
//----------------------------------------------------------------------------------------
template<class T> bool OpcodeTable<T>::RegisterOpcode(T* opcode)
{
...
//----------------------------------------------------------------------------------------
template<class T> bool OpcodeTable<T>::AllocateRegionToOpcode(T* opcode, const std::wstring& definition, const std::wstring& substitutions)
{
	size_t p1;
	size_t p2;
...
//----------------------------------------------------------------------------------------
template<class T> T* OpcodeTable<T>::GetInstruction(unsigned int opcode) const
{
