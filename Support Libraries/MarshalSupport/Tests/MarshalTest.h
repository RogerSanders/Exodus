...
class MarshalTest :public IMarshalTestBase
{
public:
	//In tests
...
protected:
	...
	virtual void OutTestVectorExtensions01(const Marshal::Out<std::vector<int>>& marshaller) const
	{
		marshaller.reserve(testData.testVectorExtensions01.size());
		for(int i = 0; i < (int)testData.testVectorExtensions01.size(); ++i)
		{
			marshaller.push_back(testData.testVectorExtensions01[i]);
		}
	}
	virtual void OutTestVectorExtensions02(const Marshal::Out<std::vector<CustomMarshalObject>>& marshaller) const
	{
		marshaller.reserve(testData.testCustomMarshaller01.size());
		for(int i = 0; i < (int)testData.testCustomMarshaller01.size(); ++i)
		{
			marshaller.push_back(testData.testCustomMarshaller01[i]);
		}
	}
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	virtual void OutTestVectorExtensions03(const Marshal::Out<std::vector<CustomMarshalObject>>& marshaller) const
	{
		marshaller.reserve(testData.testCustomMarshaller01.size());
		for(int i = 0; i < (int)testData.testCustomMarshaller01.size(); ++i)
		{
			CustomMarshalObject value = testData.testCustomMarshaller01[i];
			marshaller.push_back(std::move(value));
		}
	}
	virtual void OutTestVectorExtensions04(const Marshal::Out<std::vector<MoveTypeNoCopy>>& marshaller) const
	{
		std::vector<MoveTypeNoCopy> data = testData.GetTestVectorExtensions03();
		for(int i = 0; i < (int)data.size(); ++i)
		{
			marshaller.push_back(std::move(data[i]));
		}
	}
#endif
	virtual void OutTestVectorExtensions11(const Marshal::Out<std::vector<int>>& marshaller) const
	{
		marshaller.AssignFrom(&testData.testVectorExtensions01[0], testData.testVectorExtensions01.size());
	}
	virtual void OutTestVectorExtensions12(const Marshal::Out<std::vector<CustomMarshalObject>>& marshaller) const
	{
		std::vector<CustomMarshalObject> data = testData.testCustomMarshaller01;
		marshaller.AssignFrom(&data[0], data.size());
	}
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	virtual void OutTestVectorExtensions14(const Marshal::Out<std::vector<MoveTypeNoCopy>>& marshaller) const
	{
		std::vector<MoveTypeNoCopy> data = testData.GetTestVectorExtensions03();
		marshaller.AssignFromWithMove(data.data(), data.size());
	}
#endif

#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	//Move only object tests
	virtual bool InTestNoCopy01(const Marshal::In<MoveTypeNoCopy>& marshaller) const
	{
		MoveTypeNoCopy test = marshaller.Get();
		REQUIRE(test.GetValue() == 42);
		return true;
	}
	virtual bool InTestNoCopy02(const Marshal::In<std::vector<MoveTypeNoCopy>>& marshaller) const
	{
		std::vector<MoveTypeNoCopy> test = marshaller.Get();
		REQUIRE(test.size() == (size_t)2);
		REQUIRE(test[0].GetValue() == 1);
		REQUIRE(test[1].GetValue() == 2);
		return true;
	}
	virtual bool InTestNoCopy03(const Marshal::In<std::list<MoveTypeNoCopy>>& marshaller) const
	{
		std::list<MoveTypeNoCopy> test = marshaller.Get();
		REQUIRE(test.size() == (size_t)2);
		auto i = test.begin();
		REQUIRE(i++->GetValue() == 1);
		REQUIRE(i->GetValue() == 2);
		return true;
	}
	virtual void OutTestNoCopy01(const Marshal::Out<MoveTypeNoCopy>& marshaller) const
	{
		MoveTypeNoCopy test(4321);
		marshaller = std::move(test);
		marshaller = MoveTypeNoCopy(1234);
	}
	virtual void OutTestNoCopy02(const Marshal::Out<std::vector<MoveTypeNoCopy>>& marshaller) const
	{
		std::vector<MoveTypeNoCopy> test;
		test.push_back(MoveTypeNoCopy(1));
		test.push_back(MoveTypeNoCopy(2));
		marshaller = std::move(test);
	}
	virtual void OutTestNoCopy03(const Marshal::Out<std::list<MoveTypeNoCopy>>& marshaller) const
	{
		std::list<MoveTypeNoCopy> test;
		test.push_back(MoveTypeNoCopy(1));
		test.push_back(MoveTypeNoCopy(2));
		marshaller = std::move(test);
	}
	virtual Marshal::Ret<MoveTypeNoCopy> RetTestNoCopy01() const
	{
		return MoveTypeNoCopy(5678);
	}
	virtual Marshal::Ret<std::vector<MoveTypeNoCopy>> RetTestNoCopy02() const
	{
		std::vector<MoveTypeNoCopy> test;
		test.push_back(MoveTypeNoCopy(1));
		test.push_back(MoveTypeNoCopy(2));
	...
	virtual Marshal::Ret<std::list<MoveTypeNoCopy>> RetTestNoCopy03() const
	{
		std::list<MoveTypeNoCopy> test;
		test.push_back(MoveTypeNoCopy(1));
		test.push_back(MoveTypeNoCopy(2));
	...
	virtual bool InTestMoveConstructOnly01(const Marshal::In<MoveTypeConstructOnly>& marshaller) const
	{
		MoveTypeConstructOnly test = marshaller.Get();
		REQUIRE(test.GetValue() == 42);
		return true;
	}
	...
	//std::pair tests
	virtual bool InTestPair01(const Marshal::In<std::pair<std::wstring, int>>& marshaller) const
	{
		return CompareTestData(marshaller.Get(), testData.testPair01);
	}
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	virtual bool InTestPair02(const Marshal::In<std::pair<std::wstring, MoveTypeNoCopy>>& marshaller) const
	{
		return CompareTestData(marshaller.Get(), testData.testPair02);
	}
#endif
	virtual void OutTestPair01(const Marshal::Out<std::pair<std::wstring, int>>& marshaller) const
	{
		marshaller = testData.testPair01;
	}
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	virtual void OutTestPair02(const Marshal::Out<std::pair<std::wstring, MoveTypeNoCopy>>& marshaller) const
	{
		MarshalTestData testDataCopy;
		marshaller = std::move(((MarshalTestData&)testDataCopy).testPair02);
	}
#endif
	virtual Marshal::Ret<std::pair<std::wstring, int>> RetTestPair01() const
	{
		return testData.testPair01;
	}
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	virtual Marshal::Ret<std::pair<std::wstring, MoveTypeNoCopy>> RetTestPair02() const
	{
		MarshalTestData testDataCopy;
		return std::move(((MarshalTestData&)testDataCopy).testPair02);
	}
#endif

#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	//std::tuple tests
	virtual bool InTestTuple01(const Marshal::In<std::tuple<std::wstring, int, bool, std::string>>& marshaller) const
	{
		return CompareTestData(marshaller.Get(), testData.testTuple01);
	}
	virtual bool InTestTuple02(const Marshal::In<std::tuple<std::string, MoveTypeNoCopy>>& marshaller) const
	{
		return CompareTestData(marshaller.Get(), testData.testTuple02);
	}
	virtual bool InTestTuple03(const Marshal::In<std::tuple<>>& marshaller) const
	{
		return CompareTestData(marshaller.Get(), testData.testTuple03);
	}
	virtual void OutTestTuple01(const Marshal::Out<std::tuple<std::wstring, int, bool, std::string>>& marshaller) const
	{
		marshaller = testData.testTuple01;
	}
	virtual void OutTestTuple02(const Marshal::Out<std::tuple<std::string, MoveTypeNoCopy>>& marshaller) const
	{
		MarshalTestData testDataCopy;
		marshaller = std::move(((MarshalTestData&)testDataCopy).testTuple02);
	}
	virtual void OutTestTuple03(const Marshal::Out<std::tuple<>>& marshaller) const
	{
		marshaller = testData.testTuple03;
	}
	virtual Marshal::Ret<std::tuple<std::wstring, int, bool, std::string>> RetTestTuple01() const
	{
		return testData.testTuple01;
	}
	virtual Marshal::Ret<std::tuple<std::string, MoveTypeNoCopy>> RetTestTuple02() const
	{
		MarshalTestData testDataCopy;
		return std::move(((MarshalTestData&)testDataCopy).testTuple02);
	}
	virtual Marshal::Ret<std::tuple<>> RetTestTuple03() const
	{
		return testData.testTuple03;
	}
#endif

#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	//STL pointer tests
	virtual bool InTestPointer01(const Marshal::In<std::unique_ptr<int, MarshalSupport::MarshallableDelete<int>>>& marshaller) const
	{
		REQUIRE(*marshaller.Get().get() == 42);
		return true;
	}
	virtual bool InTestPointer02(const Marshal::In<std::unique_ptr<ISomeInterface>>& marshaller) const
	{
		REQUIRE(marshaller.Get().get()->SomeMethod() == 42);
		return true;
	}
	virtual void OutTestPointer01(const Marshal::Out<std::unique_ptr<int, MarshalSupport::MarshallableDelete<int>>>& marshaller) const
	{
		marshaller.Set(new int(42));
	}
	virtual void OutTestPointer02(const Marshal::Out<std::unique_ptr<ISomeInterface>>& marshaller) const
	{
		marshaller.Set(CreateSomeInterface());
	}
	virtual Marshal::Ret<std::unique_ptr<int, MarshalSupport::MarshallableDelete<int>>> RetTestPointer01() const
	{
		return new int(42);
	}
	virtual Marshal::Ret<std::unique_ptr<ISomeInterface>> RetTestPointer02() const
	{
		return CreateSomeInterface();
	}
#endif
