...
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
#include "IMarshalTestCPP11.h"
typedef IMarshalTestCPP11 IMarshalTestBase;
#else
typedef IMarshalTest IMarshalTestBase;
#endif
...
//----------------------------------------------------------------------------------------
extern "C" __declspec(dllexport) bool MarshalTestVectorExtensions(IMarshalTestBase& marshalTest)
{
	MarshalTestData testData;

	std::vector<int> testExtensions01;
	std::vector<CustomMarshalObject> testExtensions02;
	std::vector<int> testExtensions11;
	std::vector<CustomMarshalObject> testExtensions12;
	marshalTest.OutTestVectorExtensions01(testExtensions01);
	marshalTest.OutTestVectorExtensions02(testExtensions02);
	marshalTest.OutTestVectorExtensions11(testExtensions11);
	marshalTest.OutTestVectorExtensions12(testExtensions12);

	bool result = true;
	result &= CompareTestData(testData.testVectorExtensions01, testExtensions01);
	result &= CompareTestData(testData.testCustomMarshaller01, testExtensions02);
	result &= CompareTestData(testData.testVectorExtensions01, testExtensions11);
	result &= CompareTestData(testData.testCustomMarshaller01, testExtensions12);

#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	if(marshalTest.Version() > 0)
	{
		std::vector<CustomMarshalObject> testExtensions03;
		std::vector<MoveTypeNoCopy> testExtensions04;
		std::vector<MoveTypeNoCopy> testExtensions14;
		marshalTest.OutTestVectorExtensions03(testExtensions03);
		marshalTest.OutTestVectorExtensions04(testExtensions04);
		marshalTest.OutTestVectorExtensions14(testExtensions14);
		result &= CompareTestData(testData.testCustomMarshaller01, testExtensions03);
		result &= CompareTestData(testData.GetTestVectorExtensions03(), testExtensions04);
		result &= CompareTestData(testData.GetTestVectorExtensions03(), testExtensions14);
	}
#endif
...
//----------------------------------------------------------------------------------------
extern "C" __declspec(dllexport) bool MarshalTestNoCopyTypes(IMarshalTestBase& marshalTest)
{
	MarshalTestData testData;

	bool result = true;
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	std::vector<MoveTypeNoCopy> inTestNoCopy02;
	inTestNoCopy02.push_back(MoveTypeNoCopy(1));
	inTestNoCopy02.push_back(MoveTypeNoCopy(2));
	std::list<MoveTypeNoCopy> inTestNoCopy03;
	inTestNoCopy03.push_back(MoveTypeNoCopy(1));
	inTestNoCopy03.push_back(MoveTypeNoCopy(2));
	MoveTypeNoCopy outTestNoCopy01(0);
	std::vector<MoveTypeNoCopy> outTestNoCopy02;
	std::list<MoveTypeNoCopy> outTestNoCopy03;
	...
	result &= marshalTest.InTestNoCopy01(MoveTypeNoCopy(42));
	result &= marshalTest.InTestNoCopy02(std::move(inTestNoCopy02));
	result &= marshalTest.InTestNoCopy03(std::move(inTestNoCopy03));
	marshalTest.OutTestNoCopy01(outTestNoCopy01);
	marshalTest.OutTestNoCopy02(outTestNoCopy02);
	marshalTest.OutTestNoCopy03(outTestNoCopy03);
	//##FIX## Visual Studio 2013 contains an optimizer bug with move only types, where it unsafely inlines code across
	//assembly boundaries. This causes these tests to fail in the "DLL Release" build mode. The bug isn't present in
	//later releases of Visual Studio. See if we can find a workaround, or put in alternate special case behaviour for
	...
	REQUIRE(outTestNoCopy02.size() == (size_t)2);
	REQUIRE(outTestNoCopy02[0].GetValue() == 1);
	REQUIRE(outTestNoCopy02[1].GetValue() == 2);
	REQUIRE(outTestNoCopy03.size() == (size_t)2);
	auto outTestNoCopy03Iterator = outTestNoCopy03.begin();
...
//----------------------------------------------------------------------------------------
extern "C" __declspec(dllexport) bool MarshalTestPair(IMarshalTestBase& marshalTest)
{
	MarshalTestData testData;

	bool result = true;
	std::pair<std::wstring, int> outTestPair01;
	result &= marshalTest.InTestPair01(testData.testPair01);
	marshalTest.OutTestPair01(outTestPair01);
	std::pair<std::wstring, int> retTestPair01(marshalTest.RetTestPair01());

	MarshalTestData originalTestData;
	result &= CompareTestData(originalTestData.testPair01, outTestPair01);
	result &= CompareTestData(originalTestData.testPair01, retTestPair01);

#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	if(marshalTest.Version() > 0)
	{
		std::pair<std::wstring, MoveTypeNoCopy> outTestPair02(std::make_pair(std::wstring(L""), MoveTypeNoCopy(0)));
		result &= marshalTest.InTestPair02(std::move(testData.testPair02));
		marshalTest.OutTestPair02(outTestPair02);
		std::pair<std::wstring, MoveTypeNoCopy> retTestPair02(marshalTest.RetTestPair02().Get());
		result &= CompareTestData(originalTestData.testPair02, outTestPair02);
		result &= CompareTestData(originalTestData.testPair02, retTestPair02);
	}
#endif
...
//----------------------------------------------------------------------------------------
extern "C" __declspec(dllexport) bool MarshalTestTuple(IMarshalTestBase& marshalTest)
{
	MarshalTestData testData;

	bool result = true;
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	std::tuple<std::wstring, int, bool, std::string> outTestTuple01;
	std::tuple<std::string, MoveTypeNoCopy> outTestTuple02(std::make_tuple(std::string(""), MoveTypeNoCopy(0)));
	std::tuple<> outTestTuple03;
	result &= marshalTest.InTestTuple01(testData.testTuple01);
	result &= marshalTest.InTestTuple02(std::move(testData.testTuple02));
	result &= marshalTest.InTestTuple03(testData.testTuple03);
	marshalTest.OutTestTuple01(outTestTuple01);
	marshalTest.OutTestTuple02(outTestTuple02);
	marshalTest.OutTestTuple03(outTestTuple03);
	std::tuple<std::wstring, int, bool, std::string> retTestTuple01(marshalTest.RetTestTuple01().Get());
	std::tuple<std::string, MoveTypeNoCopy> retTestTuple02(marshalTest.RetTestTuple02().Get());
	std::tuple<> retTestTuple03(marshalTest.RetTestTuple03().Get());

	MarshalTestData originalTestData;
	result &= CompareTestData(originalTestData.testTuple01, outTestTuple01);
	result &= CompareTestData(originalTestData.testTuple02, outTestTuple02);
	result &= CompareTestData(originalTestData.testTuple03, outTestTuple03);
	result &= CompareTestData(originalTestData.testTuple01, retTestTuple01);
	result &= CompareTestData(originalTestData.testTuple02, retTestTuple02);
	result &= CompareTestData(originalTestData.testTuple03, retTestTuple03);
#endif
	return result;
}

//----------------------------------------------------------------------------------------
extern "C" __declspec(dllexport) bool MarshalTestUniquePtr(IMarshalTestBase& marshalTest)
{
	MarshalTestData testData;

	bool result = true;
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	std::unique_ptr<int, MarshalSupport::MarshallableDelete<int>> retTestPointer01 = marshalTest.RetTestPointer01().Get();
	REQUIRE(*(retTestPointer01.get()) == 42);
	std::unique_ptr<ISomeInterface> retTestPointer02 = marshalTest.RetTestPointer02().Get();
	REQUIRE(retTestPointer02->SomeMethod() == 42);
#endif
...
//----------------------------------------------------------------------------------------
extern "C" __declspec(dllexport) bool MarshalTestInPrimitive(IMarshalTestBase& marshalTest)
{
	MarshalTestData testData;
...
//----------------------------------------------------------------------------------------
extern "C" __declspec(dllexport) bool MarshalTestInComplex(IMarshalTestBase& marshalTest)
{
	MarshalTestData testData;
...
//----------------------------------------------------------------------------------------
extern "C" __declspec(dllexport) bool MarshalTestOutPrimitive(IMarshalTestBase& marshalTest)
{
	MarshalTestData testData;
	bool result = true;
...
//----------------------------------------------------------------------------------------
extern "C" __declspec(dllexport) bool MarshalTestOutComplex(IMarshalTestBase& marshalTest)
{
	MarshalTestData testData;
	bool result = true;
...
//----------------------------------------------------------------------------------------
extern "C" __declspec(dllexport) bool MarshalTestInOutPrimitive(IMarshalTestBase& marshalTest)
{
	MarshalTestData testData;
...
//----------------------------------------------------------------------------------------
extern "C" __declspec(dllexport) bool MarshalTestInOutComplex(IMarshalTestBase& marshalTest)
{
	MarshalTestData testData;
...
//----------------------------------------------------------------------------------------
extern "C" __declspec(dllexport) bool MarshalTestRetPrimitive(IMarshalTestBase& marshalTest)
{
	MarshalTestData testData;
	bool result = true;
...
//----------------------------------------------------------------------------------------
extern "C" __declspec(dllexport) bool MarshalTestRetComplex(IMarshalTestBase& marshalTest)
{
	MarshalTestData testData;
	bool result = true;
...
extern "C" __declspec(dllexport) ISomeInterface* CreateSomeInterface()
{
	return new SomeInterface();
}

extern "C" __declspec(dllexport) IPerformanceTest* CreatePerformanceTestInterface()
{
	return new PerformanceTest();
}

extern "C" __declspec(dllexport) void DeletePerformanceTestInterface(IPerformanceTest* target)
{
	delete target;
}
