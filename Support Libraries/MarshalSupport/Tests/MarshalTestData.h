...
public:
...
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	,testPair02(std::make_pair(std::wstring(L"Test"), MoveTypeNoCopy(1234)))
	,testTuple02(std::make_tuple(std::string("Test"), MoveTypeNoCopy(1234)))
#endif
	{
		//In tests
		...
		testVectorExtensions01.reserve(10000);
		for(unsigned int i = 0 ; i < 10000; ++i)
		{
			testVectorExtensions01.push_back(i);
		}

		testPair01 = std::make_pair(std::wstring(L"Test"), 1234);
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
		testTuple01 = std::make_tuple(std::wstring(L"Test"), 1234, true, std::string("MoreData"));
#endif
	}
	...
	std::vector<int> testVectorExtensions01;
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	std::vector<MoveTypeNoCopy> GetTestVectorExtensions03() const
	{
		MoveTypeNoCopy init[] = { MoveTypeNoCopy(1), MoveTypeNoCopy(2), MoveTypeNoCopy(3) };
		std::vector<MoveTypeNoCopy> result{std::make_move_iterator(std::begin(init)), std::make_move_iterator(std::end(init))};
		return result;
	}
#endif

	std::pair<std::wstring, int> testPair01;
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	std::pair<std::wstring, MoveTypeNoCopy> testPair02;

	std::tuple<std::wstring, int, bool, std::string> testTuple01;
	std::tuple<std::string, MoveTypeNoCopy> testTuple02;
	std::tuple<> testTuple03;
#endif
};
